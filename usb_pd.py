...
#     def extract_toc_pages(self) -> List[str]:
#         """Extract the likely Table of Contents pages from the PDF"""
#         toc_pages = []
#         with pdfplumber.open(self.pdf_path) as pdf:
#             for page in pdf.pages:
#                 text = page.extract_text()
#                 if text and ("Table of Contents" in text or "CONTENTS" in text):
#                     toc_pages.append(text)
#                     # Also get a few following pages that might contain the rest of ToC
#                     next_pages = pdf.pages[page.page_number:page.page_number+5]
#                     for next_page in next_pages:
#                         toc_pages.append(next_page.extract_text())
#                     break
#         return toc_pages
    
#     def parse_toc_entry(self, line: str) -> Optional[Dict]:
#         """Parse a single line from the Table of Contents"""
#         pattern = r"""
#             ^\s*                          # Optional whitespace at start
#             (?P<section_id>\d+(?:\.\d+)*)  # Section number (e.g. 2.1.2)
#             \s+                            # Whitespace between number and title
#             (?P<title>[^\n.]+)             # Title (everything until dots or newline)
#             \s*\.{3,}\s*                  # Dots separator
#             (?P<page>\d+)                  # Page number
...
#         # Infer hierarchy level from number of dots in section_id
#         level = section_id.count('.') + 1
        
#         # Determine parent_id by removing last part of section_id
#         parent_id = None
#         if '.' in section_id:
#             parts = section_id.split('.')
#             parent_id = '.'.join(parts[:-1])
#             if not parent_id:  # Handle case like "2." becoming empty string
#                 parent_id = parts[0]
...
#         """Parse the entire Table of Contents from the PDF"""
#         toc_pages = self.extract_toc_pages()
#         if not toc_pages:
...
#             for line in page_text.split('\n'):
#                 entry = self.parse_toc_entry(line)
#                 if entry:
#                     entries.append(entry)
        
#         # Sort entries by section_id to ensure hierarchy is correct
#         entries.sort(key=lambda x: [int(part) for part in x['section_id'].split('.')])
#         self.toc_entries = entries
#         return entries
    
#     def save_as_jsonl(self, output_path: str):
#         """Save the parsed Table of Contents as a JSONL file"""
...
# def main():
#     # Set up argument parser
#     parser = argparse.ArgumentParser(description='Parse USB PD Specification PDF Table of Contents')
    
#     # Use your specific PDF path as the default
#     default_pdf_path = r"D:\CTS\USB PD3 CTS Q2 2025 OR\USB PD R2.0 V1.3 2017-11-17\USB_PD_R2_0 V1.3.pdf"
#     parser.add_argument('--input_pdf', default=default_pdf_path,
#                        help=f'Path to the USB PD specification PDF file (default: {default_pdf_path})')
#     parser.add_argument('--output', default='usb_pd_spec.jsonl', 
...
#         print(f"Processing PDF: {args.input_pdf}")
#         parser = USBPDSpecParser(args.input_pdf)
#         toc_entries = parser.parse_toc()
#         parser.save_as_jsonl(args.output)
#         print(f"Successfully parsed {len(toc_entries)} ToC entries to {args.output}")
